...
const DiverseServicesSection = () => {
    const { data } = useItemRegistrationStore((state) => state);
    const [isOpen, setIsOpen] = useState(false);
    const [selectedItems, setSelectedItems] = useState([]);

    const filteredData = data.filter(item => item.code.startsWith('3'));
    ...
    return (
        <>
            <DividerTitle title="serviços diversos" clickFunction={() => setIsOpen(true)} />
